...
class MonteCarloTreeSearch(object):
    ...
    def parallel_helper(self, simulations_number):
    ...
    def best_action(self, simulations_number=1, c_param=1.4):
        """
        Parameters
        ----------
        ...
        for _ in range(number_of_processes):
            p = Process(target=self.parallel_helper, args=(int(simulations_number/number_of_processes), ))
            processes.append(p)
            p.start()
        for p in processes:
            p.join()
